...
data <- data_null %>%
  ...
  mutate(pass_end = last(p_value) < alpha) %>%
  mutate(pass_anywhere = min(p_value) < alpha)

data_wait <- data %>%
  filter(day == day_num, p_value < alpha)
...
```{r}
data %>%
  filter(head(cumsum(c(1, p_value) < alpha), -1) == 0) %>%
