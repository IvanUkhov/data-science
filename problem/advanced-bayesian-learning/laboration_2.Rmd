...
```{r}
geom_reference <- function(data, type = 'cdf', binwidth = 1, color = 'gray70') {
  if (type == 'cdf') {
    stat_ecdf(data = data,
              mapping = aes(x),
              color = color)
...
```{r}
sample_Ptheta_prior <- function(l, mu0 = 0, kappa0 = 1, nu0 = 4, sigma0 = 1) {
...
sample_Ptheta_posterior <- function(l, x, mu0 = mean(x), kappa0 = 1, nu0 = 4, sigma0 = sd(x)) {
  n1 <- length(x)
  if (n1 == 0) {
    mu1 <- 0
    ss1 <- 0
...
summarize_inference <- function(draws,
  ...
  sample <- evaluate_Pm(draws, grid, type = type) %>%
    ...
    summarize(y_mean = mean(y),
              ...
              .groups = 'drop')
  ggplot() +
    geom_reference(reference, type = type, color = 'gray90', ...) +
    geom_ribbon(data = sample,
                mapping = aes(x, ymin = y_lower, ymax = y_upper),
                fill = 'royalblue3',
                alpha = 0.25) +
...
draws <- sample_DPM(data$x, 25, 1000, kappa0 = 0.01, prior_only = TRUE)
