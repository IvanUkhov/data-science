...
plot_distribution <- function(sample, reference) {
  sample <- sample %>%
    ...
    mutate(p = cumsum(p))
  ggplot() +
    stat_ecdf(data = reference, mapping = aes(x), color = 'gray70') +
...
state <- list(
  ...
  theta = sample_P0_prior(m, mu0 = mean(data$x))
)
...
P2_distribution <- function(draw, grid) {
  bind_cols(draw$theta, tibble(p = draw$p)) %>%
    ...
    mutate(data = map2(mu, tau, function(mu, tau) {
                                  tibble(x = grid,
                                         y = pnorm(grid, mean = mu, sd = sqrt(1 / tau)))
                                })) %>%
    unnest(data) %>%
    group_by(x) %>%
...
predictive_check <- function(draws, reference, size = 1000) {
  ...
  sample <- sapply(1:length(draws),
                   simplify = FALSE,
                   function(i) {
                     P2_distribution(draws[[i]], grid) %>%
                       mutate(.draw = i) %>%
