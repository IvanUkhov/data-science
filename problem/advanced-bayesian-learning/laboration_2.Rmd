...
sample_gibbs <- function(x, m, l,
  ...
  state <- list(
    ...
    theta = tibble(mu = rep(mean(x), m),
                   tau = rep(1 / var(x) / 2.5^2, m))
  )
  draws <- vector('list', l)
  for(i in 1:l) {
    state$k <- update_k(x, state)
    state$p <- update_p(x, state, nu0 = nu0)
...
draws <- sample_gibbs(data0$x, 10, 1000, sigma0 = 1)
predictive_check(sample(tail(draws, length(draws) / 2), 10), data0, type = 'pdf')
```
...
draws <- sample_gibbs(data$x, 10, 1000)
