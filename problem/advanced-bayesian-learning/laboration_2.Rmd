...
draws <- vector('list', L)
for(i in 1:L) {
  state$k <- update_k(data$Velocity, state)
  state$p <- update_p(data$Velocity, state)
  state$theta <- update_theta(data$Velocity, state)
  draws[[i]] <- state
}
```

```{r}
tibble(count = sapply(tail(draws, L / 2), function(draw) length(unique(draw$k)))) %>%
  ggplot(aes(count)) +
  geom_density()
```
