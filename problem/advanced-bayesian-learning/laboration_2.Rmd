...
nu_sample <- function(n, nu) {
  x <- nu$P0.sample(n)
  v <- c(rbeta(n - 1, shape1 = 1, shape2 = nu$nu0), 1)
...
nu_prior <- function(mu0 = 0, n0 = 1, alpha0 = 1, beta0 = 0.5) {
  P0.sample <- function(n) {
    tau <- rgamma(n, alpha0, beta0)
    mu <- rnorm(n, mu0, n0 * tau)
    tibble(mu = mu, tau = tau)
  }
  list(P0.sample = P0.sample)
}

nu_posterior <- function(x, mu0 = 0, n0 = 1, alpha0 = 1, beta0 = 0.5) {
  n1 <- length(x)
  mu1 <- mean(x)
...
```{r}
nu_prior(mu0 = mean(data$Velocity))$P0.sample(1000) %>%
  ggplot() +
  geom_density(aes(mu))
```

```{r}
nu_posterior(data$Velocity, mu0 = mean(data$Velocity))$P0.sample(1000) %>%
  ggplot() +
  geom_density(aes(mu))
```
...
state <- list(
  ...
  theta = nu_prior(mu0 = mean(data$Velocity))$P0.sample(m)
)
