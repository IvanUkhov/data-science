...
```{r}
theta_prior_measure <- function(mu0 = 0, n0 = 1, alpha0 = 1, beta0 = 0.5) {
  sample <- function(n) {
...
theta_posterior_measure <- function(x, mu0 = 0, n0 = 1, alpha0 = 1, beta0 = 0.5) {
  n1 <- length(x)
  mu1 <- mean(x)
...
```{r}
theta_prior_measure(mu0 = mean(data$Velocity))$sample(1000) %>%
  as_tibble() %>%
  ggplot() +
  geom_density(aes(mu))
```
...
```{r}
theta_posterior_measure(data$Velocity, mu0 = mean(data$Velocity))$sample(1000) %>%
  as_tibble() %>%
  ggplot() +
  geom_density(aes(mu))
```
