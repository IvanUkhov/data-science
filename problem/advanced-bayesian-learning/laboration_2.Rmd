...
sample_gibbs <- function(x, m, l, mu0 = mean(x), sigma0 = 2.5 * sd(x), n0 = 2) {
  state <- list(
    p = rep(1 / m, m),
    k = sample(1:m, length(x), replace = TRUE),
    theta = tibble(mu = rep(mean(x), m), tau = rep(1 / var(x), m))
  )
  draws <- vector('list', l)
  for(i in 1:l) {
...
draws <- sample_gibbs(data0$x, 2, 1000, mu0 = 0, sigma0 = 5)
```
