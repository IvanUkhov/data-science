...
```{r}
sample_P0_prior <- function(l, mu0 = 0, n0 = 1, alpha0 = n0 / 2, beta0 = n0 / 2) {
  tau <- rgamma(l, alpha0, beta0)
  mu <- rnorm(l, mu0, 1 / sqrt(n0 * tau))
  tibble(mu = mu, tau = tau)
}

sample_P0_posterior <- function(l, x, mu0 = 0, n0 = 1, alpha0 = n0 / 2, beta0 = n0 / 2) {
  n1 <- length(x)
  if (n1 == 0) {
    mu1 <- 0
    s1 <- 0
...
sample_lambda_prior <- function(l, p, alpha0 = 1, beta0 = 1) {
  rgamma(l, alpha0, beta0)
}

sample_lambda_posterior <- function(l, p, alpha0 = 1, beta0 = 1) {
  sample_lambda_prior(
    l = l,
    alpha0 = alpha0 + length(p) - 1,
    beta0 = beta0 - log(tail(p, 1)),
  )
}

evaluate_P2 <- function(x, theta) {
  dnorm(x, theta$mu, 1 / sqrt(theta$tau))
}

evaluate_P3_ <- function(draw, grid, type = 'cdf') {
  if (type == 'cdf') {
    method <- pnorm
  } else if (type == 'pdf') {
    method <- dnorm
...
evaluate_P3 <- function(draws, grid, ...) {
  process <- function(i) {
    evaluate_P3_(draws[[i]], grid, ...) %>%
      mutate(.draw = i) %>%
...
update_k <- function(x, state) {
  ...
  p <- matrix(evaluate_P2(x, theta), nrow = m, ncol = n)
  p <- sweep(p, 1, state$p, `*`)
...
update_lambda <- function(x, state, ...) {
  sample_lambda_posterior(1, state$p, ...)
}
...
sample_DPM <- function(x, m, l,
  ...
  state <- list(
    lambda = lambda,
    p = stick_break(m, 1, lambda),
    k = sample(1:m, length(x), replace = TRUE),
    theta = tibble(mu = rep(mu0, m),
                   tau = rep(1 / sigma0^2, m))
  )
  draws <- vector('list', l)
  for(i in 1:l) {
    state$k <- update_k(x, state)
    state$p <- update_p(x, state)
    state$theta <- update_theta(x, state,
                                mu0 = mu0,
                                n0 = n0,
                                alpha0 = n0 / 2,
                                beta0 = n0 * sigma0^2 / 2)
...
predictive_check <- function(draws, reference, size = 1000, type = 'cdf', ...) {
  ...
  sample <- evaluate_P3(draws, grid, type = type)
  ggplot() +
...
evaluate_P3_(list(p = c(0.3, 0.7), theta = tibble(mu = c(-5, 5), tau = c(1, 1))),
...
predictive_check(sample(tail(draws, length(draws) / 2), 10), data0,
                 type = 'histogram')
```
...
predictive_check(sample(tail(draws, length(draws) / 2), 10), data,
                 type = 'histogram')
```