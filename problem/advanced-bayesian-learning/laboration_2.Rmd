...
stick_break <- function(l, alpha, beta) {
  ...
  q <- c(head(q, -1), 1)
  list(p = q * c(1, cumprod(1 - head(q, -1))))
}
...
sample_Plambda_posterior <- function(l, p, alpha0 = 2, beta0 = 0.1) {
...
update_lambda <- function(x, state, ...) {
  sample_Plambda_posterior(1, state$stick$p, ...)
}
...
sample_DPM <- function(x, m, l,
  ...
  state <- list(
    ...
    theta = tibble(mu = rep(mu0_x, m),
                   tau = rep(1 / sigma0_x^2, m))
  )
  draws <- vector('list', l)
  for(i in 1:l) {
    ...
    state$theta <- update_theta(x, state,
                                ...
                                n0 = n0,
                                alpha0 = n0 / 2,
                                beta0 = n0 * sigma0_x^2 / 2)
    state$lambda <- update_lambda(x, state,
                                  alpha0 = n0,
...
evaluate_Pm_(list(stick = list(p = c(0.3, 0.7)),
                  theta = tibble(mu = c(-5, 5), tau = c(1, 1))),
