...
```{r}
model %>%
  spread_draws(noise_intercept, noise_slope[dimension], process_sigma, process_ell) %>%
  ungroup() %>%
  select(-dimension) %>%
  pivot_longer(noise_intercept:process_ell) %>%
  mutate(name = factor(name, levels = c('noise_intercept', 'noise_slope', 'process_sigma', 'process_ell')),
         name = fct_rev(name)) %>%
  ggplot(aes(value, name)) +
  stat_pointintervalh() +
...
model %>%
  ...
  transmute(curve = pmap(list(noise_intercept = noise_intercept,
                              noise_slope = noise_slope), sample)) %>%
  unnest(curve) %>%
  group_by(x) %>%
  mean_qi() %>%
  ggplot(aes(x, y)) +
...
posterior_predictive <- function(x_new, x, y, noise_intercept, noise_slope, ...) {
  ...
  K_21 <- kernel(x_new, x, ...);
  K_22 <- kernel(x_new, x_new, ...);
  U <- chol(K_11 + diag(0.01 + 0 * exp(noise_intercept + x %*% noise_slope)));
  U_inv <- backsolve(U, diag(m));
  K_inv <- U_inv %*% t(U_inv);
  mu_new <- K_21 %*% K_inv %*% y;
  U_new <- chol(K_22 - K_21 %*% K_inv %*% t(K_21) + diag(0.01 + 0 * exp(noise_intercept + x_new %*% noise_slope)));
  as.vector(mu_new + U_new %*% as.matrix(rnorm(n)))
}
...
model %>%
  ...
  mutate(curve = pmap(list(noise_intercept = noise_intercept,
